...
module Examples.KnapSackNames where
...
type Value  = Double
type Table l = [(Name,Capacity,l)]
type Path  l = View l [Name]

class Semiring r => KS l r where
  result :: (Name,l) -> r

  ks :: Table l -> DP (Int,Capacity) r 
  ks t (0,_) = one
  ks t (_,0) = one 
  ks t (k,b) 
    | wk > b    = memo(k-1,b)
...
triple1 = zip3 names1 capacity1 values1

triple2:: [(String,Double,[Double])]
triple2 = zip3 names1 capacity1 values2
...
ks6 = knapSack triple1 150 :: Path (Small Double)
